document.getElementById('fetchAppointmentsBtn').addEventListener('click', () => {
    ...
    .then(([patients, doctors, consultationRooms, appointments]) => {
      ...
      appointments.forEach(appointment => {
        ...
        row.innerHTML = `
          ...
          <td>
            <button class="edit-button" onclick='editAppointment(${JSON.stringify(appointment)})'>Editar</button>
            <button class="delete-button" onclick='deleteAppointment(${appointment.id})'>Eliminar</button>
          </td>
        `;
        appointmentsList.appendChild(row);
      });
      appointmentsTable.style.display = appointments.length ? 'table' : 'none';
    })
  ...
  document.getElementById('appointmentForm').addEventListener('submit', event => {
    ...
    const consultationRoom_id = document.getElementById('consultationRoom_id').value;
    ...
    fetch(url, {
      ...
      body: JSON.stringify({ patient_id, doctor_id, consultationRoom_id, date, time, status })
    })
      .then(handleApiError)
      .then(() => {
  ...
  function editAppointment(appointment) {
    ...
    document.getElementById('consultationRoom_id').value = appointment.consultationRoom_id;
    ...
    fetchSelectOptions('/api/consultationRooms', 'consultationRoom_id', appointment.consultationRoom_id);
  }
